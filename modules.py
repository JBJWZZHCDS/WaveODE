...
def kaiserSincFilter(cutOff,halfWidth,kernelSize):
    ...
    filter=2*cutOff*torch.sinc(2*cutOff*time)*window # Kaiser Window with Sinc filter
    if cutOff!=0:
        filter/=filter.sum()
...
class DownSampler(nn.Module):
    ...
    def forward(self,x):
        ...
        x=F.conv1d(x,self.filter.expand(C,-1,-1),stride=self.ratio,groups=C) # Channel-wise filtering
        return x
...
class UpSampler(nn.Module):
    ...
    def forward(self,x):
        ...
        x=self.ratio*F.conv_transpose1d(x,self.filter.expand(C,-1,-1),stride=self.ratio,groups=C)
...
class Block(nn.Module):
    def __init__(self,channels,kernelSize=3,dilation=1):
        ...
        #self.conv3=nn.Conv1d(channels,2*channels,1)
    ...
    def forward(self,x):
        ...
        #x1,x2=torch.chunk(self.conv3(x),chunks=2,dim=1)
...
#         super().__init__()
#         self.step=step
#         self.scale=nn.Parameter(torch.randn(step-1))
        
#     def forward(self):
#         scale=torch.sigmoid(self.scale)
#         timeScheduler=[torch.tensor([1.0],device=scale.device)]
#         for i in range(self.step-1):
#             timeScheduler.append(timeScheduler[-1]*scale[i])
#         timeScheduler.append(torch.tensor([0.0],device=scale.device))
#         return 1-torch.cat(timeScheduler,dim=0)                
