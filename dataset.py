...
def audio2Mel(audioPath,melSavingPath,device='cuda:0'):  
    ...
    for audioName in tqdm(files,desc='Transforming audios'):
        ...
        melSpectrogram=melSpectrogram.clamp(min=0.01).log2().squeeze(0)
...
class AudioMelSet(Dataset):
    ...
    def __getitem__(self,index):
        waveform, sampleRate=torchaudio.load(self.audioPath+'/'+self.nameList[index]+'.wav')
        ...
        if melSpectrogram.size(-1)<melLen:
            ...
            melSpectrogram=F.pad(melSpectrogram,(0,melLen-melSpectrogram.size(-1)),mode='replicate')
            waveform=F.pad(waveform,(0,audioLen-waveform.size(-1)),mode='replicate')
            return waveform,melSpectrogram
        else:
            start=torch.randint(low=melSpectrogram.size(-1)-melLen,high=melSpectrogram.size(-1)-melLen+1,size=(1,)).item()
            end=start+melLen
            melWindow=melSpectrogram[...,start:end]
            if waveform.size(-1)<end*params['hopSize']:
                waveform=F.pad(waveform,(0,end*params['hopSize']-waveform.size(-1)),mode='replicate')
                
            audioWindow=waveform[...,start*params['hopSize']:end*params['hopSize']]
...
if __name__=='__main__':
    ...
    parser = argparse.ArgumentParser(description='Transform audios into mel-spectrograms.')
    parser.add_argument('-i','--audioPath',type=str,default='C:/deep_learning/LibriTTS/train',help='Audio files\' path.')
    parser.add_argument('-o','--melSavingPath',type=str,default='./trainMels',help='Mel-spectrograms\' saving path.')
    parser.add_argument('-d','--device',type=str,default='cuda:0',help='Processing device (cpu/cuda:0)')
