...
class AudioMelSet(Dataset):
    ...
    def __getitem__(self,index):
        ...
        if melSpectrogram.size(-1)<melLen:
            ...
            melSpectrogram=F.pad(melSpectrogram,(0,melLen-melSpectrogram.size(-1)),mode='constant',value=0)
            waveform=F.pad(waveform,(0,audioLen-waveform.size(-1)),mode='replicate')
            return waveform,melSpectrogram
        else:
            start=torch.randint(low=0,high=melSpectrogram.size(-1)-melLen+1,size=(1,)).item()
            end=start+melLen
            melWindow=melSpectrogram[...,start:end]
            if waveform.size(-1)<end*params['hopSize']:
                waveform=F.pad(waveform,(0,end*params['hopSize']-waveform.size(-1)),mode='constant',value=0)
